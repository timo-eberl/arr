...
func _spawn_one_ship() -> bool:
	...
	# alle existierenden Schiffe holen
	var ships := get_tree().get_nodes_in_group("enemy_ship")

	# Spawnpunkte filtern, die weit genug von existierenden Schiffen weg sind
	var free_spawns: Array[Node3D] = []
	for sp in spawn_list:
		var sp_pos: Vector3 = (sp as Node3D).global_transform.origin
		var too_close := false
	...
	if free_spawns.is_empty():
		# kein Spawnpunkt ist frei genug -> diesmal nichts spawnen
		print("Kein freier Spawnpunkt (alle Schiffe zu nah).")
		return false

	# zufälligen freien Spawnpunkt wählen
